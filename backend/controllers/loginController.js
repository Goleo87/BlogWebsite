...
export const loginPost = async (req, res, next) => {
  ...
  try {
    ...
    if (!foundUser) {
      return next(createError(401, "Login unsuccessful - please check your details"));
    }
  ...
  if (foundUser) {
    ...
    const refreshToken = jwt.sign({ id: foundUser.id }, process.env.SECRET_KEY, { expiresIn: "7d" });
